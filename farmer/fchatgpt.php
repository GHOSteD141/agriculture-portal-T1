...
// Safely pass the PHP environment variable to JavaScript
...
function fetchMessages() {
    ...
    $.ajax(settings).done(function(response) {
        ...
        const md5Hash = CryptoJS.MD5(htmlText).toString();
        
        const botMessageHtml = '<pre><div class="message left-side" id="' + md5Hash + '">' + htmlText + '</div><i class="far fa-clipboard ml-1 btn btn-outline-dark" id="' + md5Hash + '-copy"></i></pre>';             

        chatbox.append(botMessageHtml); 

        // Add event listener to the copy icon 
        var copyIcon = document.getElementById(md5Hash + '-copy'); 
        ...
        // 4. Safely check for the error message to prevent the undefined crash
        let errorMessageText = "Network or CORS Error. Check your API Key and billing status.";
        if (jqXHR.responseJSON && jqXHR.responseJSON.error && jqXHR.responseJSON.error.message) {
            errorMessageText = jqXHR.responseJSON.error.message;
        }
        
        let errorMessageHtml = '<pre><div class="message left-side text-danger"><b>Error:</b> ' + errorMessageText + '</div></pre>';
        chatbox.append(errorMessageHtml);
        chatbox.animate({ scrollTop: 20000000 }, "slow");
    });
}
</script>
